...
- Add a new **Wiki** button to the game settings, Torg Eternity banner button.
...
- The format of the various `CONFIG.torgeternity.xxxTraits` objects have changed - so any custom changes will need to use the new format.
...
- Add `test.shock` and `test.wounds` as additional changes allowed in Active Effects, to modify the amount of wounds/shock generated by the attack.
...
**NOTE** : `Transfer on Outcome` really requires two separate entries:
  ...
  - Only apply the changes to the check result on a specific outcome
...
- Game System option to display a **Effects Panel** for the currently selected token:
  ...
  - _Right click_ on an effect to delete it from the actor (or disable if on an item owned by the actor).
