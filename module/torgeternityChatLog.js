import { renderSkillChat, TestResult, rollBonusDie, torgDamage } from './torgchecks.js';
...
export default class TorgeternityChatLog extends foundry.applications.sidebar.tabs.ChatLog {
  ...
  async #showInspector(event, li) {
    ...
    doField('', game.messages.get(li.dataset.messageId).system);
 ...
 */
  static async #onApplySoak(event, button) {
    ...
    const origtest = origmsg.system.toObject();
...
function getMessage(button) {
  ...
  const test = chatMessage.system.toObject();
